...
const BookingFlight = () => {
        ...
        const fetchFlightDetails = async () => {
            try {
                ...
                if ( response.data.success ) {
                    ...
                    if ( !selectedCoach && flightData.coaches && flightData.coaches.length > 0 ) {
                        setSelectedCoach( flightData.coaches[ 0 ] );
                    }
    ...
    // Fetch available seats for the selected coach
    const fetchAvailableSeats = async ( flightId, coachId ) => {
        try {
            const response = await axiosInstance.get( `/flight/${ flightId }/seats`, {
                params: { coachId }
            } );
    ...
    const getStations = () => {
        if ( !flight || !flight.stations || flight.stations.length === 0 ) {
    ...
    const handleBookingSubmit = async () => {
        ...
        try {
            ...
            // Submit booking
            const response = await axiosInstance.post( `/booking/flight/${ id }`, bookingData );
                        ...
                        <Paper elevation={3} sx={{ p: 3, mb: 4 }}>
                            ...
                            <Box sx={{ mt: 2 }}>
                                ...
                                <Box display="flex" gap={1} sx={{ mt: 1 }}>
                                    {selectedCoach && selectedCoach.coachType === 'Economy' && (
                                        <Chip size="small" label="15kg baggage allowance" />
                                    )}
                                    {selectedCoach && selectedCoach.coachType === 'Business' && (
                                        <>
                                            <Chip size="small" label="30kg baggage allowance" />
                                            <Chip size="small" label="Meals included" />
                                            <Chip size="small" label="Wi-Fi access" />
                                        </>
                                    )}
                                    {selectedCoach && selectedCoach.coachType === 'First Class' && (
                                        <>
                                            <Chip size="small" label="40kg baggage allowance" />
                                            <Chip size="small" label="Premium meals" />
                                            <Chip size="small" label="High-speed Wi-Fi" />
                                            <Chip size="small" label="Power outlets" />
                                        </>
                                    )}
                                </Box>
                            </Box>
                    ...
                    <Box>
                        <Paper elevation={3} sx={{ p: 3, mb: 4 }}>
                            ...
                            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                                Please select seats for each passenger. Seats shown in green are available.
                            </Typography>
    ...
    return (
        <Container maxWidth="lg" sx={{ py: 4 }}>
            ...
            <Typography variant="h4" component="h1" gutterBottom>
                {activeStep === 4 ? 'Booking Confirmation' : 'Book Your Flight'}
            </Typography>
            ...
            >
                ...
                <DialogContent>
                    <DialogContentText>
                        You are about to book {passengers.length} ticket(s) for {flight?.flightName} from {departureStation?.stationName} to {arrivalStation?.stationName} for a total of {formatPrice( calculateTotalPrice() )}.
                    </DialogContentText>
